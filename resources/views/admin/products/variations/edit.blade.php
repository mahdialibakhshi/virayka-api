    ...
    <div class="row">
        ...
        <div class="col-xl-12 col-md-12 mb-4 p-4 bg-white">
            <div class="mb-4 text-center">
                @if(file_exists(public_path(env('PRODUCT_IMAGES_THUMBNAIL_UPLOAD_PATH').$variation->primary_image))
                          and !is_dir(public_path(env('PRODUCT_IMAGES_THUMBNAIL_UPLOAD_PATH').$variation->primary_image)))
                    <img class="img-thumbnail mb-2"
                         src="{{ asset(env('PRODUCT_IMAGES_THUMBNAIL_UPLOAD_PATH').$variation->primary_image) }}">
                @endif
                <h5 class="font-weight-bold mb-3 mb-md-0">{{ $variation->Product->name }} - ویرایش محصول </h5>
            </div>
            <hr>

            @include('admin.sections.errors')

            <form action="{{ route('admin.product.variations.update',['variation'=>$variation->id]) }}" method="POST"
                  enctype="multipart/form-data">
                @csrf
                @method('put')
                <input name="product_id" type="hidden" value="{{ $variation->Product->id }}">
                <div class="form-row">
                    <div class="form-group col-12 col-md-3">
                        ...
                        <select id="attribute_id" name="attribute_id" class="form-control">
                            ...
                            @foreach($attributes as $attribute)
                                <option
                                    {{ $variation->attribute_id==$attribute->id ? 'selected' : '' }} value="{{ $attribute->id }}">{{ $attribute->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div id="attribute_input_value" class="form-group col-12 col-md-3">
                        ...
                        <select class="form-control" id="attribute_product_input" name="attribute_input_value">
                            @foreach($attribute_values as $attribute_value)
                                <option
                                    {{ $variation->value==$attribute_value->id ? 'selected' : '' }} value="{{ $attribute_value->id }}">{{ $attribute_value->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group col-12 col-md-3">
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <input id="percentSalePrice" type="number" name="percentSalePrice"
                                   value="{{ $variation->percentSalePrice }}" class="form-control">
                        </div>
                        <div class="form-group col-md-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" id="variationInputDateOnSaleFrom"
                                       ...
                                       value="{{ $variation->date_on_sale_to==null ? null : verta($variation->date_on_sale_from)->format('Y-m-d') }}">
                            </div>
                        </div>
                        <div class="form-group col-md-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" id="variationInputDateOnSaleTo"
                                       ...
                                       value="{{ $variation->date_on_sale_to==null ? null : verta($variation->date_on_sale_to)->format('Y-m-d') }}">
                            </div>
                        ...
                        <div class="col-md-12">
                            <div class="alert alert-info d-flex justify-content-between">
                                ...
                                <input type="checkbox" name="showOnIndex"
                                       id="showOnIndex" {{ $variation->showOnIndex==1 ? 'checked' : '' }}>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="alert alert-dark d-flex justify-content-between">
                                ...
                                <input type="checkbox" name="has_discount"
                                       id="has_discount" {{ $variation->has_discount==1 ? 'checked' : '' }}>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <a href="{{ route('admin.product.variations.index',['product'=>$variation->Product->id]) }}"
                   class="btn btn-dark mt-5 mr-3">بازگشت</a>
                @if(file_exists(public_path(env('PRODUCT_IMAGES_THUMBNAIL_UPLOAD_PATH').$variation->primary_image))
                          and !is_dir(public_path(env('PRODUCT_IMAGES_THUMBNAIL_UPLOAD_PATH').$variation->primary_image)))
                    <a href="{{ route('admin.product.variations.remove_image',['variation'=>$variation->id]) }}"
                       class="btn btn-danger mt-5 mr-3">پاک کردن تصویر محصول</a>
                @endif
            </form>
        </div>
